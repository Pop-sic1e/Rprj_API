...
# edges 데이터에 존재하는 모든 doi 벡터 생성
edges <- read.csv('./data/edges_2.csv') # Rstudio 용량 문제로 필요하면 전부 삭제 하면, 다시 불러오기를 위한 코드
cited_doi <- unique(edges$cited)
citing_doi <- unique(edges$citing)
node_doi_vec <- unique(c(cited_doi, citing_doi))
...
nodes <- make_node(node_doi_vec)
# Save the data.frame to a CSV file
write.csv(nodes, file = "./data/nodes_2.csv", row.names = FALSE)


# DOI 정리 - 첫번째 사이클
edges <- read.csv('./data/edges_2.csv') # 재호출
nodes <- read.csv('./data/nodes_2.csv')
edges <- edges[,2:3] # csv의 index 열 제거
